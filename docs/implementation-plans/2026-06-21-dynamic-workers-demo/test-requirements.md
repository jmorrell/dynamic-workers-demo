...
| AC | Type | Test location | What it verifies |
|----|------|---------------|------------------|
| AC2.1 | integration **+ deploy** | `test/examples/examples.spec.ts`; **deploy** if linkedom can't run in-isolate | `markdown` (defuddle) returns markdown/clean text for an article fixture/URL |
...
- [ ] **AC2.1** — confirm `markdown` returns non-empty output in-isolate (deploy or `happy-dom` if `linkedom` incompatible).
