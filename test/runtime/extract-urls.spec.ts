...
import { extractLinkedResources } from '../../src/runtime/extract-urls';

const resourceUrls = (body: string, contentType: string, baseUrl: string): string[] =>
	extractLinkedResources(body, contentType, baseUrl).map((resource) => resource.url);

describe('extractLinkedResources', () => {
	const base = 'https://example.com/page';

	describe('HTML content', () => {
		it('preserves the element and attribute that granted each resource', () => {
			const resources = extractLinkedResources(
				'<a href="/page">page</a><img src="/image.png">',
				'text/html',
				base,
			);
			expect(resources).toEqual([
				{ url: 'https://example.com/page', source: { kind: 'html', element: 'a', attribute: 'href' } },
				{ url: 'https://example.com/image.png', source: { kind: 'html', element: 'img', attribute: 'src' } },
			]);
		});
		it('collects URL-bearing attributes across element types', () => {
			...
			`;
			const urls = resourceUrls(html, 'text/html', base);
		...
		it('resolves relative URLs against the base URL', () => {
			...
			const urls = resourceUrls(html, 'text/html', 'https://example.com/dir/page');
		...
		it('caps the number of extracted URLs at 2000', () => {
			...
			const urls = resourceUrls(anchors, 'text/html', base);
			expect(urls.length).toBe(2000);
		});

		it('handles application/xhtml+xml as HTML', () => {
			const html = `<a href="https://xhtml.example/a">a</a>`;
			const urls = resourceUrls(html, 'application/xhtml+xml; charset=utf-8', base);
	...
	describe('JSON / text content', () => {
		it('extracts absolute http(s) URLs from JSON, including escaped forward slashes', () => {
			...
			const urls = resourceUrls(json + escaped, 'application/json', base);
		...
		it('extracts URLs from plain text and trims trailing punctuation', () => {
			...
			const urls = resourceUrls(text, 'text/plain', base);
