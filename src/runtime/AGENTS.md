...
- **Exposes**: `runInLoader(env, input, code, runId, ctx, opts?) → RunResult`
  ...
  `{ compatDate?, extraModules?, wasmModules?, permissions?, initialResources? }`); `fetchTarget(url,
  opts?) → FetchOutcome`; `verifyTurnstile(...)`; `transpileUserCode(source) →
  TranspileResult`; `extractLinkedResources(body, contentType, baseUrl) → ResourceDescriptor[]`;
...
- **run() contract**: the harness exposes `run(input: RunInput): Promise<RunResult>`
  ...
  `{ resources: Map<url, { url, source, read() }> }`. Each zero-argument `read()`
  closes over an opaque capability id and proxies to the host `CapabilityGate`.
- **Permissions**: `type Permissions = { fetch: 'page-links' | 'none'; cpuMs?;
  ...
  unlocks `env.DB` and REQUIRES the run request to carry a uuid `storeId`
...
- **Storage contract** (`storage: 'scoped'`): a storage-granted run routes
  ...
  `env.DB = { databaseSize, exec }` over the facet's `ctx.storage.sql`. Each
  query is materialized inside `transactionSync`; growth beyond 128 KiB throws
  and rolls back. The cap constant lives in core.ts and is interpolated into
  `HARNESS_SOURCE`, whose cap logic mirrors `checkDatabaseSize`. A rejected
  write throws a catchable Error, consistent
...
- **Facet restart for per-run telemetry**: a live facet retains the class from
  its first `facets.get()` startup callback, including that Dynamic Worker's
  tail binding and captured `runId`. Before each storage invocation the
  supervisor calls `ctx.facets.abort(storeKey, reason)`, then `get()` remounts
  the current per-run class. `abort` preserves the facet's SQLite database, so
  persistence survives while code and log routing are refreshed.
...
- **Gate contract**: `CapabilityGate` (WorkerEntrypoint, reached via
  `ctx.exports.CapabilityGate({ props: { runId, initialResources, fetchDepth,
  maxFetches } })`, attached as the loaded worker's `env.GATE`).
  `readResource(opaqueId)` resolves the host-owned URL and returns either a text
  result (2 MiB, UTF-8-boundary truncation, plus newly minted child grants) or a
  bytes result (20 MiB, byte truncation), selected from the actual response
  content type. Reads enforce capability ownership, SSRF guards, an 8s timeout,
  and the shared per-run cap. Only successful text reads mint child capabilities,
  and only while the granted `fetchDepth` permits another hop.
- **URL extraction**: `extractLinkedResources` (pure, `extract-urls.ts`) parses HTML
  ...
  http/https only, dedupes, caps at 2000, and preserves advisory source metadata.
...
- **`runInLoader`'s `wasmModules`** (`Record<string, Uint8Array>`) are injected
  ...
  overload) works with no runtime compilation step. `src/index.ts` accepts at
  most 4 custom source/wasm modules with safe names that cannot shadow
  `harness.js`/`user.js`, and applies an 8 MiB cap per source or decoded binary
  (see `validateCustomModules`/`decodeBase64` in `core.ts`). Wasm bytes come from
  custom runs (request-supplied) or example runs (the example's manifest
...
- **Gate read counter is module-scoped, not instance state**: workerd
  instantiates a fresh `WorkerEntrypoint` per RPC call, so a per-instance counter
  would reset each `read()`. The gate keeps a module-level `Map<runId, count>`
...
- **SYNC PARTNER**: `harness-source.ts` inlines `classifyTransformError` as a
  string, and inlines the `env.DB` cap-check logic mirroring core's
  `checkDatabaseSize` (the numeric limit is interpolated from core's
  `STORE_MAX_*` constants, but the check ordering/logic is hand-copied);
  `core.ts` is canonical for both. Change both together.

## Invariants
- Loaded worker `env` carries the `CapabilityGate` loopback and plain initial
  resource grants only under a page-links grant; otherwise it stays `{}`. Never pass host
...
- Containment limits: `CPU_LIMIT_MS = 50` (overridable per-run via clamped
  ...
  read caps bullet below — `globalOutbound: null` (always).
- Gate read caps: text 2 MiB (UTF-8-boundary), bytes 20 MiB (byte),
  8s timeout, `maxFetches` resource reads per run (default 5, clamped `[1, 100]` —
  ...
  `subRequests` stays at the old constant 5. Note: gate reads execute in the
  ...
  many pages can ever contribute child capabilities). `releaseGateRun(runId)` (called
  from `src/index.ts`'s `handleRun` after logs are read and gate spans are
  drained) deletes a run's entries from the read-count, child-resource, and
  gate-span maps — best-effort hygiene; correctness never depends on it running.
- Every resource read records a trace span draft — denials
...
- `extract-urls.ts` - pure `extractLinkedResources` (resource discovery from a page)
